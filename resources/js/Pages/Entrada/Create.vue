...
const form = useForm({
    ...
    gestion_id: props.gestiones.length > 0 ? props.gestiones[0].id : '',
})

// ✅ Usar esta función al hacer submit
function submit() {
    form.post(route('asistencia.storeentrada'), {
        preserveScroll: true,
    })
}
</script>
    ...
    <div>
        ...
        <form @submit.prevent="submit" class="bg-white p-6 rounded shadow space-y-4">
            ...
            <div>
                ...
                <select v-model="form.gestion_id"
                    class="w-full mt-1 px-4 py-2 border rounded shadow-sm focus:ring focus:ring-blue-200">
                    <option disabled value="">Seleccione una gestión</option>
                    <option v-for="g in gestiones" :key="g.id" :value="g.id">{{ g.nombre }}</option>
                </select>
                <div v-if="form.errors.gestion_id" class="text-red-500 text-sm mt-1">{{ form.errors.gestion_id }}</div>
            </div>
            ...
            <div>
                ...
                <div v-if="form.errors.tipoalerta_id" class="text-red-500 text-sm mt-1">{{ form.errors.tipoalerta_id }}
                </div>
            </div>
